...
class MapView: MAMapView, MAMapViewDelegate {
 ...
 @objc var onCallback: RCTBubblingEventBlock = { _ in }
   ...
   // 设置自定义地图样式
   @objc var customStyleOptions: [String: Any]? {
       didSet {
           if let styleOptions = customStyleOptions, styleOptions.isEmpty {
               customMapStyleEnabled = false
               return
           }
           
           // 使用闭包而不是 DispatchWorkItem
           DispatchQueue.global(qos: .background).async { [weak self] in
               ...
               // 处理 styleData
               if let styleData = self.customStyleOptions?["styleData"] as? [String: Any],
                 let uriString = styleData["uri"] as? String,
                 let url = URL(string: uriString) {
                   do {
                       let data = try Data(contentsOf: url)
                       options.styleData = data
                   } catch {
                       print("加载 styleData 失败: \(error)")
                   }
               }
               
               // 处理 styleExtraData
               if let styleExtraData = self.customStyleOptions?["styleExtraData"] as? [String: Any],
                 let uriString = styleExtraData["uri"] as? String,
                 let url = URL(string: uriString) {
                   do {
                       let data = try Data(contentsOf: url)
                       options.styleExtraData = data
                   } catch {
                       print("加载 styleExtraData 失败: \(error)")
                   }
               }
               
               // 处理 styleTextureData
               if let styleTextureData = self.customStyleOptions?["styleTextureData"] as? [String: Any],
                 let uriString = styleTextureData["uri"] as? String,
                 let url = URL(string: uriString) {
                   do {
                       let data = try Data(contentsOf: url)
                       options.styleTextureData = data
                   } catch {
                       print("加载 styleTextureData 失败: \(error)")
                   }
               }
               
               // 处理 styleId
               if let styleId = self.customStyleOptions?["styleId"] as? String {
...
//                    self.update(self.locationRender)
 ...
 override func didAddSubview(_ subview: UIView) {
   ...
   }else if let marker = subview.contentView, let annotation = (marker as? Marker)?.annotation {
         markerMap[annotation] = marker as? Marker
         addAnnotation(annotation)
    }
 }
...
extension UIView {
    ...
    var contentView: UIView? {
        ...
        set {
            // 将新值关联到 self 对象上
            // .OBJC_ASSOCIATION_RETAIN_NONATOMIC: 表示强引用，非原子操作
