...
- (void)speakString:(NSString *)string
{
    ...
    dispatch_async(self.serialQueue, ^{
        ...
        if (self.speechSynthesizer)
        {
            ...
            if ([self.delegate respondsToSelector:@selector(speechSynthesizer:updateIsSpeaking:)]) {
                [self.delegate speechSynthesizer:self updateIsSpeaking:YES];
            }
        }
    });
}
...
- (void)stopSpeak
{
    if (self.speechSynthesizer)
    {
        ...
        if ([self.delegate respondsToSelector:@selector(speechSynthesizer:updateIsSpeaking:)]) {
            [self.delegate speechSynthesizer:self updateIsSpeaking:NO];
        }
    }
}
...
{
    if ([self.delegate respondsToSelector:@selector(speechSynthesizer:updateIsSpeaking:)]) {
        [self.delegate speechSynthesizer:self updateIsSpeaking:[self isSpeaking]];
    }
